...
export function JapanMap({
  ...
  const mainProjection = useMemo(() => {
    ...
    return geoMercator()
      ...
      .scale(baseScale)
      .translate([dimensions.width * 0.45, dimensions.height * 0.42]); // 少し上に
  ...
  const okinawaProjection = useMemo(() => {
    ...
    return geoMercator()
      .center([127.7, 26.2]) // 沖縄の中心座標
      .scale(okinawaScale)
      .translate([dimensions.width * 0.2, dimensions.height * 0.15]); // もうちょい右
  }, [dimensions.width, dimensions.height]);
    ...
    >
      ...
      <div className="absolute left-4 top-1/2 -translate-y-1/2 flex flex-col items-center gap-2 z-10">
        <div className="relative h-64 w-1 bg-gray-300 rounded-full">
          {/* ズームレベルの円 */}
          <div
            className="absolute w-4 h-4 bg-primary rounded-full -left-1.5 transition-all duration-200"
            style={{
              top: `${
          ...
          {Array.from(
            ...
            <div
              key={level}
              className="absolute -left-2 w-5 h-0.5 bg-gray-400"
              style={{
                top: `${((level - MIN_ZOOM) / (MAX_ZOOM - MIN_ZOOM)) * 100}%`,
              }}
            >
      ...
      >
        ...
        <g className="main-prefectures">
          {mainPrefectures.map((d, i) => {
            ...
            const fillColor = prefCode && !isSelected
            ...
            return (
              <path
                ...
                className="prefecture cursor-pointer transition-all duration-200"
                fill={fillColor}
                stroke="#ffffff"
                strokeWidth={1}
        ...
        <line
          x1={dimensions.width * 0.42}
          y1={dimensions.height * 0.12}
          x2={dimensions.width * 0.22}
          y2={dimensions.height * 0.32}
          stroke="#94a3b8"
          strokeWidth={1}
          strokeDasharray="8,4"
        />
